...
int* IndexSortBIN(Student* p, const int N, char* fname);
void PrintIndexSortedBIN(Student* p, int* I, const int N, char* fname);
...
int main()
{
	...
	do {
		...
		cout << " [3] - індексне впорядкування даних" << endl << endl;
		...
		case 2:
			...
			break;
		case 3:
			PrintIndexSortedBIN(p, IndexSortBIN(p, N, fname), N, fname);
			break;
		case 0:
			break;
		default:
...
int* IndexSortBIN(Student* p, const int N, char* fname)
{
	fstream f(fname, ios::binary); // відкрили файл для зчитування та запису
